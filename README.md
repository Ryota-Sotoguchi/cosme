...
> **2026-09-14 に発信ジャンルを「コスメ・美容」→「転職・年収・キャリア」へ切り替えた。**
> 投稿は転職を二回している会社員の目線で、求人票・年収交渉・面接・退職などの話をする（全枠リンクなし）。
> 楽天（コスメ）のリンク投稿は休止中で、商品エンジンは再利用できるよう残してある。
> 下のプロフィール文とスケジュール表は切り替え前のもの（プロフィールは本人が変更する）。
> 過去のコスメ投稿は `scripts/cleanup_old_posts.py` で片付ける（既定は Dry Run。CLAUDE.md 参照）。

もとは楽天市場の商品を、**価格・送料・内容量などの客観情報だけ**で紹介するアカウントだった。

> **コスメ買い物メモ｜コスパ美容**
> 楽天で見つけた気になるコスメを、価格・送料・内容量など公開情報ベースで整理🧴
...
| 22:30 | late | つぶやき／問いかけ／話題 をローテーション（リンクなし） | 可変 |
...
145件。楽天レスポンス解析、スコアリング、除外カテゴリー、重複防止、価格・件数の整合性、
