...
## 何を上に置くか
  ...
  * 発信ジャンルとの関連度 … このアカウントの軸（2026-09-14 から転職・年収・キャリア）
...
# 発信ジャンルの語。ここに当たるほど優先度が上がる。
#
# **識別子の BEAUTY_WORDS / is_beauty / beauty_ratio は、当初のジャンル（美容）の名残。**
# 2026-09-14 に発信ジャンルを「コスメ・美容」→「転職・年収・キャリア」へ変えたとき、
# 中身だけを入れ替えた。名前を変えると設定キー（[engagement] beauty_ratio）や
# 呼び出し元まで触ることになるので、ジャンルの切り替えでは変えていない。
BEAUTY_WORDS: tuple[str, ...] = (
    "転職", "転職活動", "転職サイト", "転職エージェント", "エージェント", "求人",
    "面接", "書類選考", "職務経歴書", "履歴書", "志望動機", "自己PR", "内定", "オファー",
    "退職", "辞めたい", "退職代行", "引き継ぎ", "有給",
    "年収", "年収交渉", "給料", "給与", "手取り", "昇給", "ボーナス", "賞与", "市場価値",
    "キャリア", "キャリアチェンジ", "未経験", "スキル", "資格", "副業", "働き方",
    "リモート", "在宅勤務", "管理職", "専門職", "会社員", "仕事", "職場", "上司",
    "人間関係", "残業", "定時", "福利厚生", "ブラック企業", "ホワイト企業", "異動", "評価",
)
...
SENSITIVE_WORDS: tuple[str, ...] = (
    ...
    "妊娠", "出産", "不妊", "生理",
    # 「副業」は 2026-09-14 から発信ジャンルの話題なので外した。稼げる系の勧誘は残す。
    "稼げ", "投資", "FX", "仮想通貨", "情報商材", "権利収入", "不労所得",
    # 職場のトラブルの渦中。当事者の強い感情があり、他人が入ると水を差すか誤解を招く
    "パワハラ", "セクハラ", "ハラスメント", "いじめ", "労基", "訴え", "解雇", "リストラ",
    "死にたい", "消えたい", "自殺", "休職", "適応障害",
)
